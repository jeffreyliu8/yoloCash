    ...
    fun handleAuthResult(result: ActivityResult, onTokenRequested: (TokenRequestResult) -> Unit) {
        ...
        when {
            response?.authorizationCode != null -> {
                ...
                authService.performTokenRequest(response.createTokenExchangeRequest()) { tokenResponse,
                                                                                         tokenEx ->
    ...
    fun loadModelAllowlist() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                modelAllowlist =
                    readModelAllowlistFromDisk(fileName = MODEL_ALLOWLIST_TEST_FILENAME)
                ...
                for (allowedModel in modelAllowlist.models) {
                    ...
                    val acceleratorList =
                        accelerators.split(",").map { it.trim() }.filter { it.isNotEmpty() }
                    ...
                    if (task.modelNames.isNotEmpty()) {
                        for (modelName in task.modelNames) {
                            ...
                            if (model == null) {
                                Log.w(
                                    TAG,
                                    "Model '${modelName}' in task '${task.label}' not found in allowlist."
                                )
                                continue
                            }
                            task.models.add(model)
    ...
    private fun getModelDownloadStatus(model: Model): ModelDownloadStatus {
        ...
        if (isModelPartiallyDownloaded(model = model)) {
            ...
            val tmpFilePath =
                model.getPath(
                    context = context,
                    fileName = "${model.downloadFileName}.$TMP_FILE_EXT"
                )
